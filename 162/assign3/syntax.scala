...
case class TypeDef( name:Label, constructors:Map[Label, Type] )
...
case class Record( fields:Map[Label, Exp] ) extends Exp
case class Access( e:Exp, field:Label ) extends Exp
case class Construct( name: Label, constructor:Label, e:Exp ) extends Exp
case class Match( e:Exp, cases: Seq[(Label, Var, Exp)] ) extends Exp
...
case class RcdT( fields:Map[Label, Type] ) extends Type
case class TypT( name:Label ) extends Type

object Parsers extends StdTokenParsers with PackratParsers {
  ...
  lexical.reserved ++= Seq("num", "bool", "let", "rec", "in", "type", "case", "of", "true", "false", "if", "then", "else", "nil", "unit")
  ...
  lazy val program: Parser[Program] = (typedef*) ~ exp ^^ {
    case tdefs ~ exp => Program(tdefs.toSet, exp)
  }

  lazy val typedef: Parser[TypeDef] = "type" ~ tyname ~ "=" ~ rep1sep(constDef, "|") flatMap {
    case _ ~ name ~ _ ~ constructors => ifDuplicates(constructors.map(_._1),
                                                     "constructor names",
                                                     TypeDef(name, constructors.toMap))
  }

  lazy val constDef: Parser[(Label, Type)] = tyname ~ tpe ^^ {
  ...
  lazy val matches: Parser[(Label, Var, Exp)] = "|" ~ pattern ~ "=>" ~ exp ^^ {
    case _ ~ pat ~ _ ~ body =>
      (pat._1, pat._2, body)
  }

  lazy val pattern: Parser[(Label, Var)] = (
  ...
  lazy val call: Parser[Call] = exp ~ "(" ~ rep1sep(exp, ",") ~ ")" ^^ {
    case f ~ _ ~ args ~ _ => Call(f, args)
  }

  lazy val fun: Parser[Fun] = "(" ~ rep1sep(variable ~ ":" ~ tpe, ",") ~ ")" ~ "=>" ~ exp flatMap {
...
object Pretty {
  ...
  def prettyCons(cons: Map[Label, Type]) = {
  ...
  def prettyCases(cases: Seq[(Label, Var, Exp)]) = {
