...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  def map[B](f: A => B): MyList[B] = foldRight(MyList[B]())((a:A,b:MyList[B]) => MyCons(f(a),b)) 

  def flatMap[B](f: A => MyList[B]): MyList[B] = map(f).foldLeft(MyList[B]())((a:MyList[B], b:MyList[B]) => a.append(b))

  def filter(pred: A => Boolean): MyList[A] = foldRight(MyList[A]())((item:A, accum:MyList[A]) => if(pred(item)) MyCons(item,accum) else accum)

  def append(other: MyList[A]): MyList[A] = foldRight(other)((item:A, accum:MyList[A]) => MyCons(item,accum))

  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    def usesRecursion(accum: B, items: MyList[A]): B = {
      if(items.isEmpty) accum else usesRecursion(f(accum, items.head), items.tail)
    }
    usesRecursion(initial, MyCons(x,xs))
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = {
    def usesRecursion(items: MyList[A], accum: B): B = {
      if(items.isEmpty) accum else usesRecursion(items.init, f(items.last, accum))
    }
    usesRecursion(MyCons(x,xs), initial)
  }

  def take(n: Int): MyList[A] = if(xs.length +1 < n) MyCons(x,xs) else MyCons(x,xs).reverse.drop(xs.length+1 - n).reverse

  def drop(n: Int): MyList[A] = {
    def usesRecursionPlusOtherShit(items:MyList[A], accum:Int): MyList[A] = {
      if(items.isEmpty) MyList() else if(accum == 0) items else usesRecursionPlusOtherShit(items.tail, accum-1)
    }
    if(xs.length +1 < n) MyList() else usesRecursionPlusOtherShit(MyCons(x,xs),n)
  }
  ...
  def init: MyList[A] = MyCons(x,xs).reverse.tail.reverse

  def reverse: MyList[A] = foldLeft(MyList[A]())((accum:MyList[A], item:A) => MyCons(item, accum))

  def last: A = MyCons(x,xs).reverse.head 
  ...
  def length: Int = foldRight(0)((a:A, i:Int) => i+1)
}
