...
object Main {
  ...
  def main( args:Array[String] ) {
    ...
    val masks = List(
      ...
      ImgMaskGUI(coord, Fractals.mandelbrot(10)),
      ImgMaskGUI(coord, Fractals.mandelbrot(50)),
      ImgMaskGUI(coord, Fractals.mandelbrot(500))
    )
...
object Images {
  ...
  // !!FILL ME IN
  def lift0[A](v:A): Img[A] = (p:Point) => v
  ...
  // !!FILL ME IN
  def lift1[A,B](f: A => B)(imgA: Img[A]): Img[B] = (p:Point) => f(imgA(p))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A,B,C](f: (A,B) => C)(imgA: Img[A])(imgB: Img[B]): Img[C] = (p:Point) => f(imgA(p),imgB(p))
  ...
  // !!FILL ME IN
  def lift3[A,B,C,D](f: (A,B,C) => D)(imgA: Img[A])(imgB: Img[B])(imgC: Img[C]): Img[D] = (p:Point) => f(imgA(p),imgB(p),imgC(p)) 
  ...
  // colour images.
  // !!FILL ME IN
  val lerpImgClr = lift3(lerpClr) _
  ...
  // !!FILL ME IN
  val overlayImgClr = lift2(overlayClr) _
  ...
  // ImgMask argument.
  // !!FILL ME IN
  val selectImgClr = lift3((b: Boolean, c1: Colour, c2: Colour) => if(b) c1 else c2) _
  ...
  // !!FILL ME IN
  val msk2clr = lift1((b: Boolean)=> if(b) Colour(0, 0, 0, 1) else Colour(1, 1, 1, 1) ) _
  ...
  // !!FILL ME IN
  val gray2clr = lift1((v: Value) => Colour(v,v,v,1)) _ 
}
...
object SpatialTransforms {
  ...
  // y coordinate.
  // !!FILL ME IN
  def translatePt(v: Vector): TransformPt = (p:Point) => Point(p.x+v.x, p.y+v.y)
  ...
  // coordinate.
  // !!FILL ME IN
  def scalePt(v: Vector): TransformPt = (p:Point) => Point(p.x*v.x, p.y*v.y)
  ...
  // !!FILL ME IN
  def rotatePt(theta: Double): TransformPt = (p:Point) => Point(p.x*cos(theta) - p.y*sin(theta), p.y*cos(theta) + p.x*sin(theta))
  ...
  // !!FILL ME IN
  def swirlPt(v: Double): TransformPt = (p:Point) => { 
    val theta = Utility.dist(p)*((2*PI)/v)
    Point(p.x*cos(theta) - p.y*sin(theta), p.y*cos(theta) + p.x*sin(theta))
 }

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TranformPt.
  // !!FILL ME IN
  def polarXf(v: TransformPlr): TransformPt = (p:Point) => plr2pt(v(pt2plr(p)))             
  ...
  // !!FILL ME IN
  def translate[A](v: Vector): (Point => A) => (Point => A) = (imga: Point => A) => (p:Point) => imga(translatePt(v.neg)(p))
  ...
  // !!FILL ME IN
  def scale[A](v: Vector): (Point => A) => (Point => A) = (imga: Point => A) => (p:Point) => imga(scalePt(v.inverse)(p))
  ...
  // !!FILL ME IN
  def rotate[A](theta: Double): (Point => A) => (Point => A) = (imga: Point => A) => (p:Point) => imga(rotatePt(-theta)(p)) 
  ...
  // !!FILL ME IN
  def swirl[A](theta: Double): (Point => A) => (Point => A) = (imga: Point => A) => (p:Point) => imga(swirlPt(-theta)(p))
}
...
object Masking {
  ...
  // !!FILL ME IN
  val intersectM = lift2((b1: Boolean, b2: Boolean) => b1&&b2) _
  ...
  // !!FILL ME IN
  val unionM = lift2((b1:Boolean, b2:Boolean) => b1 || b2) _ 
  ...
  // !!FILL ME IN
  val xorM = lift2((b1:Boolean, b2:Boolean) => (b1||b2)&&(!b1||(!b2))) _ 
...
object Fractals{    
  def zNext(c:Point)(z:Point): Point = Point(z.x*z.x - z.y*z.y + c.x, 2*z.x*z.y + c.y)
  def mandelbrot(n:Int): ImgMask = (c:Point) => {
    def mandelbrotAccum(v:Point)(w:Point)(accum:Int)(b:Boolean): Boolean = {
      if(!b) false else if(accum >= 0) mandelbrotAccum(v)(zNext(v)(w))(accum-1)(Utility.dist(w) <= 2) else Utility.dist(w) <=2
    }
    mandelbrotAccum(c)(Point(0,0))(n)(true) 
  }
}
